...
Escolhemos 6 ações de mercados diferentes para analisar a volatilidade
...
Vamos trabalhar tanto com os dados no formato de tibble quanto no formato de tsibble.
...
da_tsibble |>
  ...
  # precisa ajustar o label de quarter no gráfico
```


```{r}
da_2 <- da %>%
  select(ticker, ref_date, ret_closing_prices)
da_2$ret <- xts(petro[,-1],order.by = ymd(petro$ref_date))[-1,]
da_2$ret2 = ret**2
...
# Verificando se há heterocedasticidade condicional

```{r}
Box.test(da$price_adjusted, type = "Ljung-Box")
```
